...
'''change dir path'''
# dir='/work/07732/ajnam/ls6/asi_blender6/'
dir='/Users/jiheeandreanam/Harris_Lab/Scripts/Blender/asi_blender/'
os.chdir(dir)
if not dir in sys.path:
    sys.path.append(dir)
print('current dir = ', os.getcwd())
import blender_import_export as bie
import blender_smooth as bs
import blender_asi_functions as af
import blender_asi_helper_functions as ahf
import cgal_remesh_seg as cgal
...
if __name__== '__main__':
    ...
    #1) IMPORT 1um_rad_ref and astroAll
    #2) check that all synapses have their components

    if analysis: 
        headless = True
        collection_name = 'Collection' #define name of collection
        syn_list = [c.name for c in bpy.data.collections[collection_name].all_objects if ('c' in c.name.split('_')[0].rstrip('abc')) and ('seg' not in c.name)]

        glia_appo = list(np.arange(0.01, 0.13, 0.01))
        glia_appo = [round(i,2) for i in glia_appo]
        asi_headers = ['syn_name', 'syn_center', 'syn_dist', 
                       'asi_faces', 'asi_edges', 'asi_peri', 'asi_area',  'sp_area', 
                       'ref_rad', 'ref_area',  #reference circle dimensions to calculate conversion factor
                       'asi_centroid', 'psd_sp_centroid', 'psd_ax_centroid', 'psd_offset,' #centroids + psd offset
                       'min_asi2psd_dist', 'max_asi2psd_dist', 'avg_asi2psd_dist', #dist to psd for all verts
                       'min_asiGlia2psd_dist', 'max_asiGlia2psd_dist', 'avg_asiGlia2psd_dist', #dist to psd for asiGlia verts
                       'min_asiGlia2glia_dist', 'avg_asiGlia2glia_dist'] #dist to glia for asiGlia verts
        
        asi_headers += ['asiGlia_edges_' + str(i)] #asiGlia boundary edges idxs
        for i in glia_appo:
            asi_headers += ['asiGlia_peri_' + str(i)] #asiGlia perimeters 
        for i in glia_appo:
            asi_headers += ['asiGlia_frac_' + str(i)] #asiGlia fractions
        ...
        for syn in syn_list:
            ...
            print('segmenting only spine w/ cgal segments') 
            asi.segmentSA()
            print('asi c name = ', asi.c_name)
            print('asi ax name = ', asi.ax_name)
            print('asi sp name = ', asi.sp_name)
            print('extract asi indices')
            asi.extract_asi_idx()
            print('asi initial initial len = ', len(asi.asi_idx))
            if len(asi.asi_idx) > 0:
                ...
                asi.select_asi_faces()
                print('calcualte asi area')
                asi.calc_asi_area()
                print('asi_area = ', asi.asi_area)
            ...
            if len(asi.asi_bL_idx) > 0:
                ...
                #get psd obj
                asi.psd_sp_name = asi.c_name.split('_remesh')[0] + '_sp'
                asi.psd_ax_name = asi.c_name.split('_remesh')[0] + '_ax'
                # get psd area + centroid
                try:
                    print('get psd area + centroid')
                    asi.extract_psd_area_centroid()
                except KeyError:
                    print('Missing pyrecon c object')
                    continue
                #get asi2psd centroid offset
                print('get asi to psd offset')
                asi.extract_asi2psd_offset()
                #get asi to psd to glia distances
                print('get asi 2 psd + glia distances')
                asi.extract_asi2psd2glia_dist(mesh=True)
            else:
            ...
            #append results
            results.append(syn.split('_')[0]) #append synapse name
            results.append(asi.c_center_global) #append c obj center
            ...
            results.append(asi.asi_bL_idx) #append asi boundary loop ids
            results.append(asi.asi_peri) #append asi perimeter
            results.append(asi.asi_area) #append asi area
            results.append(asi.sp_area) #append sp area (surface area as a reference to compare against PyReconstruct)
            ...
            results.append(asi.psd_sp_centroid) #geometric centroid of spine psd
            results.append(asi.psd_ax_centroid) #geometric centroid of axon psd
            ...
            for i, j in enumerate(glia_appo):
                results.append(asi.asiGlia_bL[i]) #append asiGlia boundary loop edges idxs 
            for i, j in enumerate(glia_appo):
                results.append(asi.asiGlia_peri[i]) #append asiGlia perimeters
            for i, j in enumerate(glia_appo):
                results.append(asi.asiGlia_frac[i]) #append asiGlia frac (0.05 µm thresh)
